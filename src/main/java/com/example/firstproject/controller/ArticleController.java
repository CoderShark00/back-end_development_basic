...
public class ArticleController {
    ...
    public String index(Model model){
        // 1. 모든 데이터 가져오기
        // List<Article> articlEntityList = (List<Article>) articleRepository.findAll();  // Iterable을 List로 다운캐스팅해 문제 해결
        List<Article> articleEntityList = articleRepository.findAll(); // List 대신 ArrayList가능(업캐스팅)
        // 2. 모델에 데이터 등록하기
    ...
    public String update(ArticleForm form){
        ...
        //엔티티를 DB에 저장하기
        //기존 데이터 가져오기
        Article target = articleRepository.findById(articleEntity.getId()).orElse(null); //DB에서 데이터 찾기>>데이터 저장>>데이터가없으면 반환
        //기존데이터 값을 갱신
        if(target !=null)
